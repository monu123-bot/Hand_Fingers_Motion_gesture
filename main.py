...
cap = cv2.VideoCapture(0) # store file in cap
...
while(cap.isOpened()):
    ...
    blured = cv2.GaussianBlur(grey,value,0) # why we blur the image - to remove small motions occured in nature

    #thresholdian: Otsu's binarization method - this method is autometically calculate threshold values
    _,thresh1 = cv2.threshold(blured,127,255,cv2.THRESH_BINARY_INV+cv2.THRESH_OTSU) # all the pixels of value greater than 127 assign a value 1 and lesser are assign a value 0
    ...
    #create building rectangle around the contour 
    ...
    #with angle > 70 degrees and ignore defects
